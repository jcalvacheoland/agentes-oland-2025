...
export function buildPDFBuffer(invoiceData: any): Promise<Buffer> {
  return new Promise((resolve, reject) => {
    ...
    function drawTableManual(
      ...
      const headerBg = opts.headerBg ?? "#cccccc";
      const firstColBg = opts.firstColBg ?? "#cccccc";
    ...
    const filaPrimaTotal = [
      "Prima Total",
      ...plans.map((p: any) => formatearNumeroMoneda(p.totalPremium)),
    ];
    ...
    doc
      ...
      .opacity(0.8)
      .text(
        "Este documento corresponde a una cotización referencial. " +
